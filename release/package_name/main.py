...
from package_name.sqlite import session
from run import run
...
def startup() -> Result:
    ...
    inject.configure(lambda binder: injector_configs(binder, configs, logger, inputs, session))
    return Result.ok()


def injector_configs(binder, configs: Configs, logger: logging.Logger, inputs: Inputs, sqlite_session: Session):
    """
